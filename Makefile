...
DATABASE_URL ?= postgres://b46:b46_local_only@localhost:5432/b46_ordering?sslmode=disable
MIGRATIONS_DIR := services/ordering/migrations
export DATABASE_URL

.DEFAULT_GOAL := help

.PHONY: help setup db-up db-down db-reset migrate migrate-status run fmt vet test test-race test-integration architecture contracts migrations-check check
