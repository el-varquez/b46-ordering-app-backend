...
func run(ctx context.Context) error {
	name := strings.TrimSpace(os.Getenv("B46_BOOTSTRAP_ADMIN_NAME"))
	email := strings.TrimSpace(os.Getenv("B46_BOOTSTRAP_ADMIN_EMAIL"))
	password := os.Getenv("B46_BOOTSTRAP_ADMIN_PASSWORD")
	if name == "" || email == "" || password == "" {
		return errors.New("B46_BOOTSTRAP_ADMIN_NAME, B46_BOOTSTRAP_ADMIN_EMAIL, and B46_BOOTSTRAP_ADMIN_PASSWORD are required")
	}
	...
	admin, err := identity.BootstrapAdmin(ctx, name, email, password)
	if err != nil {
		return err
	}
	fmt.Printf("Admin account is ready: %s\n", admin.ID)
	return nil
}
